...
| 文件 | 类型 | 作用 |
|------|------|------|
| `Balance_Luntuimatlab.m` | 🧠 **核心函数** | 为给定腿长,用符号推导+牛顿力学+LQR 算出最优增益矩阵 K |
| `get_K.m` | 🔧 **编排脚本** | 批量调用核心函数,做多项式拟合,输出 C 语言数组代码 |
| `Balance_Luntuimatlab.asv` | 💾 **自动备份** | MATLAB 编辑器自动保存的旧版本 (参数略有不同) |
